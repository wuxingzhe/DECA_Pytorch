...
class BottleNeck_IR(nn.Module):
    def __init__(self, in_channel, out_channel, stride):
        ...
        # self.combine_layer = nn.Sequential(nn.BatchNorm2d(out_channel),
        #                                    nn.PReLU(out_channel))
...
def get_blocks(num_layers):
    if num_layers == 50:
        blocks = [
            get_block(in_channel=64, out_channel=64, num_units=3), # 64*64*64
            get_block(in_channel=64, out_channel=128, num_units=4), # 32*32*128
            get_block(in_channel=128, out_channel=256, num_units=14), # 16*16*256
            get_block(in_channel=256, out_channel=512, num_units=3) # 8*8*521
        ]
    elif num_layers == 100:
        blocks = [
...
class SEResNet_IR_ori(nn.Module):
    def __init__(self, num_layers, feature_dim=512, drop_ratio=0.4, mode = 'ir'):
        ...
        self.input_layer = nn.Sequential(nn.Conv2d(3, 64, (3, 3), 2, 1, bias=False),
                                         nn.BatchNorm2d(64),
                                         nn.PReLU(64),
                                         nn.Conv2d(64, 64, (3, 3), 1, 1, bias=False),
                                         nn.BatchNorm2d(64),
                                         nn.PReLU(64),
                                            nn.Conv2d(64, 64, (3, 3), 2, 1, bias=False),
                                         nn.BatchNorm2d(64),
                                         nn.PReLU(64)) # 64*64*64
    ...
    def forward(self, x):
        ...
        x = self.body(x)
        embs = self.output_layer(x)

        return embs, x

class SEResNet_IR_ori_224(nn.Module):
    def __init__(self, num_layers, feature_dim=512, drop_ratio=0.4, mode = 'ir'):
        super(SEResNet_IR_ori_224, self).__init__()
        assert num_layers in [50, 100, 152], 'num_layers should be 50, 100 or 152'
        assert mode in ['ir', 'se_ir'], 'mode should be ir or se_ir'
        blocks = get_blocks(num_layers)
        if mode == 'ir':
...
if __name__ == '__main__':
    ...
    net = SEResNet_IR_ori(100, mode='se_ir')
    print(net)
